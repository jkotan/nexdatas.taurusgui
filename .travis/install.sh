...
if [[ $2 == "2" ]]; then
    ...
    if [[ $1 == "debian10" ]]; then
	docker exec -it --user root ndts /bin/sh -c 'export DEBIAN_FRONTEND=noninteractive;  apt-get -qq update; apt-get -qq install -y python-taurus python-sardana nxselector python-nxselector'
    else
    ...
    if [[ $1 == "ubuntu20.04" ]]; then
	docker exec -it --user root ndts /bin/sh -c 'export DEBIAN_FRONTEND=noninteractive;  apt-get -qq update; apt-get -qq install -y python3-taurus python3-sardana nxselector python3-nxselector'
    else
	docker exec -it --user root ndts /bin/sh -c 'git clone https://github.com/taurus-org/taurus taurus-src; cd taurus-src'
	...
	docker exec -it --user root ndts /bin/sh -c 'export DEBIAN_FRONTEND=noninteractive;  apt-get -qq update; apt-get -qq install -y nxselector3 python3-nxselector'
    fi
fi
if [ "$?" -ne "0" ]
then
