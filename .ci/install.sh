...
if [[ $2 = "2" ]]; then
    ...
    if [ "$1" = "debian10" ] || [ "$1" = "debian11" ]|| [ "$1" = "debian12" ] || [ "$1" = "ubuntu20.04" ]|| [ "$1" = "ubuntu22.04" ]; then
	echo " "
    else
...
if [[ $2 == "2" ]]; then
    ...
    if [[ $1 == "debian10" ]]; then
	docker exec  --user root ndts /bin/sh -c 'export DEBIAN_FRONTEND=noninteractive;  apt-get -qq update; apt-get -qq install -y python-taurus python-sardana nxselector python-nxselector'
    else
	if [[ $1 == "debian9" ]]; then
    ...
    docker exec  --user root ndts /bin/sh -c 'export DEBIAN_FRONTEND=noninteractive;  apt-get -qq update; apt-get -qq install -y nxselector python-nxselector'
else
    echo "install sardana, taurus and nexdatas"
